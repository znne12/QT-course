    ...
    , ui(new Ui::MainWindow)
{
    ...
    connect(activSubWindow, SIGNAL(destroyed(QObject*)), this, SLOT(deleteSubWindow(QObject*)));
    isChangedWindow[textEdit] = false;
...
void MainWindow::on_action_open_triggered()
{
    ...
    sub->show();
    connect(sub, SIGNAL(destroyed(QObject*)), this, SLOT(deleteSubWindow(QObject*)));
    connect(textEdit, SIGNAL(textChanged()), this, SLOT(textChangedInWindow()));
...
bool MainWindow::needToSaveMessage(ruTextEdit *tmpTextEdit){
    if (isChangedWindow[tmpTextEdit]){
        QMessageBox save;
...
void MainWindow::saveWhenClosing(ruTextEdit *tmpTextEdit)
{
    QString s = QFileDialog::getSaveFileName(this, "Сохранить как", QDir::current().path(), filter);
    if(!s.indexOf(".txt")){
        if (s.length() > 0) if(!file->saveFile(s, tmpTextEdit->toHtml(), ui->authorEdit->text())) {
            aboutBox.setText(tr("Ошибка сохранения файла!"));
            aboutBox.exec();
        }
    }else{
        if (s.length() > 0) if(!file->saveFile(s, tmpTextEdit->toPlainText(), ui->authorEdit->text())) {
            aboutBox.setText(tr("Ошибка сохранения файла!"));
            aboutBox.exec();
        }
    }
...
{
    if (needToSaveMessage(textEdit)){   //нужно доделать для всех вкладок!
        saveWhenClosing(textEdit);
    }
    qApp->exit();
}
...
void MainWindow::changeTextEditPointer(QMdiSubWindow *window)
{
    if(mdiArea->activeSubWindow()){
        textEdit = ((ruTextEdit*)window->widget());
        ui->authorEdit->setText(textEdit->authorText);
    }
}

void MainWindow::deleteSubWindow(QObject *window)
{
    if (needToSaveMessage((ruTextEdit*)(((QMdiSubWindow*)window)->widget()))){
        saveWhenClosing((ruTextEdit*)(((QMdiSubWindow*)window)->widget()));
    }
    isChangedWindow.remove((ruTextEdit*)(((QMdiSubWindow*)window)->widget()));
}

void MainWindow::textChangedInWindow()
{
    isChangedWindow[textEdit] = true;
}
