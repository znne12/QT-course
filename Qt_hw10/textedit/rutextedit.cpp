...
ruTextEdit::ruTextEdit(QWidget *parent) : QTextEdit(parent)
{
    ...
    QAction *insertDate = menu->addAction(tr("Вставить дату"));
    QAction *insertTime = menu->addAction(tr("Вставить время"));
    ...
    connect(insertDate, &QAction::triggered, this, &ruTextEdit::insertDate);
    connect(insertTime, &QAction::triggered, this, &ruTextEdit::insertTime);
...
void ruTextEdit::insertDate()
{
    textCursor().insertText(QDate::currentDate().toString());
}

void ruTextEdit::insertTime()
{
    textCursor().insertText(QTime::currentTime().toString());
}
